...
' FILTER: Column F (Supervisor), rows 6-136
...
Sub Run_the_Screenshot()
    ...
    If supervisors.Count = 0 Then
        MsgBox "No supervisors found in column F (rows 6-136).", vbExclamation
        Exit Sub
    End If
    ...
    For i = 1 To UBound(supArray)
        ...
        If todayCol > 0 Then
            ...
            If dayoffCount <> "" And dayoffCount <> 0 Then
                dayoffCount = "_Dayoff=" & dayoffCount
            Else
                dayoffCount = ""
            End If
        End If
        ...
        ' --- 7. Take screenshot ---
        Set captureRange = ws.Range("A2:AY132")
