...
BOT_TOKEN = os.getenv("TIMELINES_BOT_TOKEN")
CHAT_ID = os.getenv("TIMELINES_CHAT_ID")

WATCH_FOLDER = os.path.join(BASE_DIR, "Timelines Screenshots")
POLL_SECONDS = 5
# ===================================================
...
def parse_filename(file_path):
    ...
    dt_part = rest
    if '_Dayoff=' in rest:
        dt_part, dayoff_str = rest.split('_Dayoff=')
        try:
...
def build_caption(supervisor, dt_list, dayoff):
    return f"=> DT៖ {dt_list}\n-ចំនួនធីមដែល Dayoff ថ្ងៃនេះ: {dayoff} ធីម"
